...
# 原始文本文件夹路径
TEXT_FOLDER_ROOT = '/root/autodl-tmp/temp/TXT'  # 替换为你的txt文件所在的文件夹路径
# 特征保存文件夹路径
FEATURE_FOLDER_ROOT = '/root/autodl-tmp/features/TEXT_features'  # 替换为你想要保存特征文件的文件夹路径
os.makedirs(FEATURE_FOLDER_ROOT, exist_ok=True)  # 如果文件夹不存在，则创建它

# 加载预训练的 BERT 中文模型和 tokenizer
tokenizer = BertTokenizer.from_pretrained("/root/autodl-tmp/code/bert_chinese")
model = BertModel.from_pretrained("/root/autodl-tmp/code/bert_chinese")
