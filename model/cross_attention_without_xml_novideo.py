...
def custom_collate_fn(batch):
    ...
    return text_batch, audio_batch, face_batch, labels

# 自定义数据集类，用于加载已经提取好的特征
class CustomDataset(Dataset):
    def __init__(self, text_data, audio_data, face_data, labels):
...
# 加载特征和标签的函数
def load_features_and_labels(text_folder, audio_folder, face_folder, annotation_csv):
    try:
    ...
    for face_path in face_files:
        ...
        # 检查所有模态的文件是否都存在
        if text_path and audio_path and face_path:
            try:
    ...
    return text_features, audio_features, face_features, labels, subsets, missing_modalities
...
# 加载特征和标签
text_features, audio_features, face_features, labels, subsets, missing_modalities = load_features_and_labels(
    text_folder, audio_folder, face_folder, annotation_csv)
...
# 检查加载的特征和标签数量
if len(text_features) == 0 or len(audio_features) == 0 or len(face_features) == 0:
...
# 构建训练集和验证集的数据集和数据加载器
train_dataset = CustomDataset(train_text, train_audio, train_face, train_labels)
test_dataset = CustomDataset(test_text, test_audio, test_face, test_labels)
...
class MultiModalCrossAttention(nn.Module):
    ...
    def forward(self, text_out, audio_out, face_out):
        # 确保输入张量的维度正确
        ...
        for _ in range(self.num_layers):
            ...
            text_out = text_out + text_audio_attn + text_face_attn
            audio_out = audio_out + text_audio_attn + audio_face_attn
            face_out = face_out + text_face_attn + audio_face_attn
        ...
        combined_out = text_out + audio_out + face_out
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (text, audio, face, label) in enumerate(train_loader):
        text, audio, face, label = text.to(device), audio.to(device), face.to(device), label.to(device)

        optimizer.zero_grad()
        output = model(text, audio, face)
...
def validation(model, device, test_loader):
    ...
    with torch.no_grad():
        for text, audio, face, label in test_loader:
            text, audio, face, label = text.to(device), audio.to(device), face.to(device), label.to(device)
            output = model(text, audio, face)
            loss = criterion(output, label.float().unsqueeze(1))
