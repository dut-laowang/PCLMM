...
def custom_collate_fn(batch):
    video_batch = [item[0].clone().detach() for item in batch]
    ...
    return video_batch, face_batch, labels

# 自定义数据集类，用于加载已经提取好的特征
class CustomDataset(Dataset):
    def __init__(self, video_data, face_data, labels):
...
# 加载特征和标签的函数
def load_features_and_labels(video_folder, face_folder, annotation_csv):
    try:
    ...
    return video_features, face_features, labels, subsets, missing_modalities
...
# 加载特征和标签
video_features, face_features, labels, subsets, missing_modalities = load_features_and_labels(
    video_folder, face_folder, annotation_csv)
...
# 检查加载的特征和标签数量
if len(video_features) == 0 or len(face_features) == 0:
...
# 构建训练集和验证集的数据集和数据加载器
train_dataset = CustomDataset(train_video, train_face, train_labels)
test_dataset = CustomDataset(test_video, test_face, test_labels)
...
class MultiModalCrossAttention(nn.Module):
    ...
    def forward(self, video_out, face_out):
        # 确保输入张量的维度正确
        ...
        for _ in range(self.num_layers):
            ...
            video_out = video_out + video_face_attn
            face_out = face_out + video_face_attn

            # 添加 Dropout
            video_out = self.dropout(video_out)
            face_out = self.dropout(face_out)

        combined_out = video_out + face_out
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (video, face, label) in enumerate(train_loader):
        video, face, label = video.to(device), face.to(device), label.to(device)

        optimizer.zero_grad()
        output = model(video, face)
        ...
        if batch_idx % log_interval == 0:
            ...
            print(
                f'Train Epoch: {epoch} [{batch_idx * len(video)}/{len(train_loader.dataset)}]\tLoss: {loss.item():.6f}\tAccuracy: {metrics["accuracy"]:.4f}')
...
def validation(model, device, test_loader):
    ...
    with torch.no_grad():
        for video, face, label in test_loader:
            video, face, label = video.to(device), face.to(device), label.to(device)
            output = model(video, face)
            loss = criterion(output, label.float().unsqueeze(1))
