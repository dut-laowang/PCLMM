...
def custom_collate_fn(batch):
    ...
    return text_batch, video_batch, face_batch, labels

# 自定义数据集类，用于加载已经提取好的特征
class CustomDataset(Dataset):
    def __init__(self, text_data, video_data, face_data, labels):
...
# 加载特征和标签的函数
def load_features_and_labels(text_folder, video_folder, face_folder, annotation_csv):
    try:
    ...
    for face_path in face_files:
        ...
        # 检查所有模态的文件是否都存在
        if text_path and video_path and face_path:
            try:
    ...
    return text_features, video_features, face_features, labels, subsets, missing_modalities
...
# 加载特征和标签
text_features, video_features, face_features, labels, subsets, missing_modalities = load_features_and_labels(
    text_folder, video_folder, face_folder, annotation_csv)
...
# 检查加载的特征和标签数量
if len(text_features) == 0 or len(video_features) == 0 or len(face_features) == 0:
...
# 构建训练集和验证集的数据集和数据加载器
train_dataset = CustomDataset(train_text, train_video, train_face, train_labels)
test_dataset = CustomDataset(test_text, test_video, test_face, test_labels)
...
class MultiModalCrossAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, num_layers, face_dim=192):
        ...
        self.cross_attention_text_face = nn.MultiheadAttention(embed_dim, num_heads, batch_first=True)
    ...
    def forward(self, text_out, video_out, face_out):
        # 确保输入张量的维度正确
        ...
        for _ in range(self.num_layers):
            ...
            text_face_attn, _ = self.cross_attention_text_face(text_out, face_out, face_out)

            video_face_attn, _ = self.cross_attention_video_face(video_out, face_out, face_out)

            text_out = text_out + text_video_attn + text_face_attn
            video_out = video_out + text_video_attn + video_face_attn
            face_out = face_out + text_face_attn + video_face_attn
        ...
        combined_out = text_out + video_out + face_out
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (text, video, face, label) in enumerate(train_loader):
        text, video, face, label = text.to(device), video.to(device), face.to(
            device), label.to(device)

        optimizer.zero_grad()
        output = model(text, video, face)
...
def validation(model, device, test_loader):
    ...
    with torch.no_grad():
        for text, video, face, label in test_loader:
            text, video, face, label = text.to(device), video.to(device), face.to(
                device), label.to(device)
            output = model(text, video, face)
            loss = criterion(output, label.float().unsqueeze(1))
