...
def custom_collate_fn(batch):
    ...
    return audio_batch, video_batch, labels
...
# 加载特征和标签的函数
def load_features_and_labels(audio_folder, video_folder, annotation_csv):
    try:
    ...
    return audio_features, video_features, labels, subsets, missing_modalities
...
# 加载特征和标签
audio_features, video_features, labels, subsets, missing_modalities = load_features_and_labels(
    audio_folder, video_folder, annotation_csv)
...
# 构建训练集和验证集的数据集和数据加载器
train_dataset = CustomDataset(train_audio, train_video, train_labels)
test_dataset = CustomDataset(test_audio, test_video, test_labels)
...
class MultiModalCrossAttention(nn.Module):
    ...
    def forward(self, audio_out, video_out):
        # 确保输入张量的维度正确
        ...
        for _ in range(self.num_layers):
            ...
            audio_out = audio_out + audio_video_attn
            video_out = video_out + audio_video_attn

            # 添加 Dropout
            audio_out = self.dropout(audio_out)
            video_out = self.dropout(video_out)

        combined_out = audio_out + video_out
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (audio, video, label) in enumerate(train_loader):
...
def validation(model, device, test_loader):
    ...
    with torch.no_grad():
        for audio, video, label in test_loader:
            audio, video, label = audio.to(device), video.to(device), label.to(device)
            output = model(audio, video)
            loss = criterion(output, label.float().unsqueeze(1))
