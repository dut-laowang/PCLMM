...
def custom_collate_fn(batch):
    ...
    return audio_batch, face_batch, labels

# 自定义数据集类，用于加载已经提取好的特征
class CustomDataset(Dataset):
    def __init__(self, audio_data, face_data, labels):
...
# 加载特征和标签的函数
def load_features_and_labels(audio_folder, face_folder, annotation_csv):
    try:
    ...
    return audio_features, face_features, labels, subsets, missing_modalities
...
# 加载特征和标签
audio_features, face_features, labels, subsets, missing_modalities = load_features_and_labels(
    audio_folder, face_folder, annotation_csv)
...
# 检查加载的特征和标签数量
if len(audio_features) == 0 or len(face_features) == 0:
...
# 构建训练集和验证集的数据集和数据加载器
train_dataset = CustomDataset(train_audio, train_face, train_labels)
test_dataset = CustomDataset(test_audio, test_face, test_labels)
...
class MultiModalCrossAttention(nn.Module):
    ...
    def forward(self, audio_out, face_out):
        # 确保输入张量的维度正确
        ...
        for _ in range(self.num_layers):
            ...
            audio_out = audio_out + audio_face_attn
            face_out = face_out + audio_face_attn

            # 添加 Dropout
            audio_out = self.dropout(audio_out)
            face_out = self.dropout(face_out)

        combined_out = audio_out + face_out
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (audio, face, label) in enumerate(train_loader):
        audio, face, label = audio.to(device), face.to(device), label.to(device)
...
def validation(model, device, test_loader):
    ...
    with torch.no_grad():
        for audio, face, label in test_loader:
            audio, face, label = audio.to(device), face.to(device), label.to(device)
            output = model(audio, face)
            loss = criterion(output, label.float().unsqueeze(1))
