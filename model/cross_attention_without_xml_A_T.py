...
# 加载特征和标签的函数
def load_features_and_labels(text_folder, audio_folder, annotation_csv):
    try:
    ...
    for text_path in text_files:
        base_name = os.path.basename(text_path).replace('.p', '')
    ...
    return text_features, audio_features, labels, subsets, missing_modalities
...
# 加载特征和标签
text_features, audio_features, labels, subsets, missing_modalities = load_features_and_labels(
    text_folder, audio_folder, annotation_csv)
...
# 检查加载的特征和标签数量
if len(text_features) == 0 or len(audio_features) == 0:
...
# 构建训练集和验证集的数据集和数据加载器
train_dataset = CustomDataset(train_text, train_audio, train_labels)
test_dataset = CustomDataset(test_text, test_audio, test_labels)
...
class MultiModalCrossAttention(nn.Module):
    ...
    def forward(self, text_out, audio_out):
        # 确保输入张量的维度正确
        ...
        for _ in range(self.num_layers):
            ...
            text_out = text_out + text_audio_attn
            audio_out = audio_out + text_audio_attn

            # 添加 Dropout
            text_out = self.dropout(text_out)
            audio_out = self.dropout(audio_out)

        combined_out = text_out + audio_out
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (text, audio, label) in enumerate(train_loader):
        text, audio, label = text.to(device), audio.to(device), label.to(device)
...
def validation(model, device, test_loader):
    ...
    with torch.no_grad():
        for text, audio, label in test_loader:
            text, audio, label = text.to(device), audio.to(device), label.to(device)
            output = model(text, audio)
            loss = criterion(output, label.float().unsqueeze(1))
