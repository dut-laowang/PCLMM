...
# 加载特征和标签的函数
def load_features_and_labels(text_folder, face_folder, annotation_csv):
    try:
    ...
    return text_features, face_features, labels, subsets, missing_modalities
...
# 加载特征和标签
text_features, face_features, labels, subsets, missing_modalities = load_features_and_labels(
    text_folder, face_folder, annotation_csv)
...
# 检查加载的特征和标签数量
if len(text_features) == 0 or len(face_features) == 0:
...
# 构建训练集和验证集的数据集和数据加载器
train_dataset = CustomDataset(train_text, train_face, train_labels)
test_dataset = CustomDataset(test_text, test_face, test_labels)
...
class MultiModalCrossAttention(nn.Module):
    ...
    def forward(self, text_out, face_out):
        # 确保输入张量的维度正确
        ...
        for _ in range(self.num_layers):
            ...
            text_out = text_out + text_face_attn
            face_out = face_out + text_face_attn

            # 添加 Dropout
            text_out = self.dropout(text_out)
            face_out = self.dropout(face_out)

        combined_out = text_out + face_out
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (text, face, label) in enumerate(train_loader):
        text, face, label = text.to(device), face.to(device), label.to(device)

        optimizer.zero_grad()
        output = model(text, face)
...
def validation(model, device, test_loader):
    ...
    with torch.no_grad():
        for text, face, label in test_loader:
            text, face, label = text.to(device), face.to(device), label.to(device)
            output = model(text, face)
            loss = criterion(output, label.float().unsqueeze(1))
