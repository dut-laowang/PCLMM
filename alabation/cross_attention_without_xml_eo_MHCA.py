...
def custom_collate_fn(batch):
    ...
    face_batch = [item[3].clone().detach() for item in batch]
    ...
    return text_batch, audio_batch, video_batch, face_batch, labels

# 自定义数据集类，用于加载已经提取好的特征
class CustomDataset(Dataset):
    def __init__(self, text_data, audio_data, video_data, face_data, labels):
    ...
    def __getitem__(self, idx):
        ...
        return text, audio, video, face, label
...
# 加载特征和标签的函数
def load_features_and_labels(text_folder, audio_folder, video_folder, face_folder, annotation_csv):
    try:
    ...
    for face_path in face_files:
        ...
        # 检查所有模态的文件是否都存在
        if text_path and audio_path and video_path and face_path:
            try:
    ...
    return text_features, audio_features, video_features, face_features, labels, subsets, missing_modalities
...
text_features, audio_features, video_features, face_features, labels, subsets, missing_modalities = load_features_and_labels(
    text_folder, audio_folder, video_folder, face_folder, annotation_csv)
...
# 检查加载的特征和标签数量
if len(text_features) == 0 or len(audio_features) == 0 or len(video_features) == 0 or len(face_features) == 0:
...
# 构建训练集和验证集的数据集和数据加载器
train_dataset = CustomDataset(train_text, train_audio, train_video, train_face, train_labels)
test_dataset = CustomDataset(test_text, test_audio, test_video, test_face, test_labels)
...
class MultiModalCrossAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, num_layers, audio_dim=40, face_dim=192):
        ...
        # 用全连接层代替 cross-attention
        self.fc_text_audio = nn.Linear(embed_dim, embed_dim)
        self.fc_text_video = nn.Linear(embed_dim, embed_dim)
        self.fc_text_face = nn.Linear(embed_dim, embed_dim)

        self.fc_audio_video = nn.Linear(embed_dim, embed_dim)
        self.fc_audio_face = nn.Linear(embed_dim, embed_dim)

        self.fc_video_face = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, text_out, audio_out, video_out, face_out):
        # 确保输入张量的维度正确
        ...
        for _ in range(self.num_layers):
            ...
            # 使用全连接层代替 cross-attention
            text_audio_attn = self.fc_text_audio(text_out)
            text_video_attn = self.fc_text_video(text_out)
            text_face_attn = self.fc_text_face(text_out)

            audio_video_attn = self.fc_audio_video(audio_out)
            audio_face_attn = self.fc_audio_face(audio_out)

            video_face_attn = self.fc_video_face(video_out)
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (text, audio, video, face, label) in enumerate(train_loader):
        text, audio, video, face, label = text.to(device), audio.to(device), video.to(device), face.to(
            device), label.to(device)

        optimizer.zero_grad()
        output = model(text, audio, video, face)
...
def validation(model, device, test_loader):
    ...
    with torch.no_grad():
        for text, audio, video, face, label in test_loader:
            text, audio, video, face, label = text.to(device), audio.to(device), video.to(device), face.to(
                device), label.to(device)
            output = model(text, audio, video, face)
            loss = criterion(output, label.float().unsqueeze(1))
